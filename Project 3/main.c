...
#define FRAME_MAX 256           /* max frames in physical memory */
...
void TLBInsert(int page_num, int frame_num) {
    int i, j;
    /* while there is space in TLB */   
    for(i = 0; i < next_tlb_pos; ++i){         
        if(TLB[i].page_num == page_num) 
        break;
    }

    /* if page number not found in TLB */
    if(i == next_tlb_pos) {
        /* if room in TLB: insert */
        if(next_tlb_pos < TLB_ENTRY_MAX) {
            TLB[next_tlb_pos].page_num = page_num;
            TLB[next_tlb_pos].frame_num = frame_num;
        } else { /* else move and insert page frame at end */
            for(i = 0; i < TLB_ENTRY_MAX-1; i++){
                TLB[i] = TLB[i+1];
            }
            TLB[TLB_ENTRY_MAX-1].page_num = page_num;
            TLB[TLB_ENTRY_MAX-1].frame_num = frame_num;
        }
    } else { /* if page num found in TLB */
        /* from page index, increment position */
        for(i = i; i < TLB_ENTRY_MAX-1; i++) {
            TLB[i] = TLB[i+1];
        }
        /* if space, add page num and frame at the end */
        if(next_tlb_pos < TLB_ENTRY_MAX){
            TLB[next_tlb_pos].page_num = page_num;
            TLB[next_tlb_pos].frame_num = frame_num;
        }
        /* else, page and frame on num entries - 1 */
        else {
            TLB[next_tlb_pos-1].page_num = page_num;
            TLB[next_tlb_pos-1].frame_num = frame_num;
        }
    }
    if(next_tlb_pos < TLB_ENTRY_MAX) {
        next_tlb_pos++;
